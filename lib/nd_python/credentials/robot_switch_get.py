...
class CredentialsRobotSwitchGet:
    ...
    Get robot switch credentials from the controller.

    ## Example robot switch get request
    ...
    def switch_username(self) -> str:
        """
        Return switchUsername from the response
        """
        method_name = inspect.stack()[0][3]
        self.error_if_not_committed(method_name)
        return self.rest_send.response_current.get("DATA", {}).get("switchUsername", "")
