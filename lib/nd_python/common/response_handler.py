...
class ResponseHandler:
    ...
    def _get_response(self) -> None:
        ...
        """
        result = {}
        if self.response.get("RETURN_CODE") == 404 and self.response.get("MESSAGE") == "Not Found":
            result["found"] = False
            result["success"] = True
        elif self.response.get("RETURN_CODE") not in self.return_codes_success or self.response.get("MESSAGE") != "OK":
            result["found"] = False
    ...
    def _post_put_delete_response(self) -> None:
        ...
        elif self.response.get("MESSAGE") != "OK" and self.response.get("MESSAGE") is not None:
            result["success"] = False
            result["changed"] = False
        else:
            result["success"] = True
